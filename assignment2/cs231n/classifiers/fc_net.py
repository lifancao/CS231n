    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == None:
                ...
                x, cache = affine_ln_relu_forward(x, w, b, gamma, beta, ln_param)
            if self.use_dropout:
                x, dropout_cache = dropout_forward(x, self.dropout_param)
                dropout_caches.append(dropout_cache)
            caches.append(cache)
            
        scores, cache = affine_forward(x, self.params['W' + \
        ...
        dx, dw, db = affine_backward(grad, caches[-1])
        grads['W' + str(self.num_layers)] = dw + self.reg * self.params['W' + \
        str(self.num_layers)]
